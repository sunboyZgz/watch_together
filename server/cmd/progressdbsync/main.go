...
var progressTable = tableSpec{
	name: "user_media_progress",
	columns: []string{
		"id", "user_id", "media_item_id", "media_episode_id", "last_position_seconds", "duration_seconds",
		"last_watched_at", "completed", "completion_source", "created_at", "updated_at",
	},
...
func parseOptions() options {
	...
	flag.StringVar(&opts.targetURL, "target-database-url", strings.TrimSpace(os.Getenv("PROGRESS_DATABASE_URL")), "target progress PostgreSQL URL; defaults to PROGRESS_DATABASE_URL")
...
func run(ctx context.Context, opts options) error {
	...
	if !opts.dryRun && strings.TrimSpace(opts.targetURL) == "" {
		return fmt.Errorf("--target-database-url or PROGRESS_DATABASE_URL is required")
	}
	if opts.verifyOnly && strings.TrimSpace(opts.targetURL) == "" {
		return fmt.Errorf("--target-database-url or PROGRESS_DATABASE_URL is required for --verify-only")
	}
	if opts.batchSize <= 0 {
		opts.batchSize = 500
	}
	...
	if opts.dryRun {
		count, err := tableCount(ctx, source, progressTable.name)
		if err != nil {
			return err
		}
		fmt.Printf("%s would copy %d rows\n", progressTable.name, count)
		return nil
	}
	copied, err := syncTable(ctx, source, target, progressTable, opts.batchSize)
	if err != nil {
		return err
	}
	fmt.Printf("%s copied %d rows\n", progressTable.name, copied)
	return nil
}
...
func verify(ctx context.Context, source *sql.DB, target *sql.DB) error {
	sourceCount, sourceHash, err := tableFingerprint(ctx, source, progressTable)
	if err != nil {
		return fmt.Errorf("source %s fingerprint: %w", progressTable.name, err)
	}
	targetCount, targetHash, err := tableFingerprint(ctx, target, progressTable)
	if err != nil {
		return fmt.Errorf("target %s fingerprint: %w", progressTable.name, err)
	}
	if sourceCount != targetCount || sourceHash != targetHash {
		...
		return fmt.Errorf("progress database verification failed")
	}
	fmt.Printf("%s ok count=%d hash=%s\n", progressTable.name, sourceCount, sourceHash)
	return nil
}
