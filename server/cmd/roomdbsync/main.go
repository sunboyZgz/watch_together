...
var roomTables = []tableSpec{
	{
		name: "rooms",
		columns: []string{
			"id", "room_code", "host_user_id", "media_episode_id", "status",
			"created_at", "updated_at", "last_empty_at", "destroy_after",
		},
...
func parseOptions() options {
	...
	flag.StringVar(&opts.targetURL, "target-database-url", strings.TrimSpace(os.Getenv("ROOM_DATABASE_URL")), "target room PostgreSQL URL; defaults to ROOM_DATABASE_URL")
...
func run(ctx context.Context, opts options) error {
	...
	if !opts.dryRun && strings.TrimSpace(opts.targetURL) == "" {
		return fmt.Errorf("--target-database-url or ROOM_DATABASE_URL is required")
	}
	if opts.verifyOnly && strings.TrimSpace(opts.targetURL) == "" {
		return fmt.Errorf("--target-database-url or ROOM_DATABASE_URL is required for --verify-only")
	}
	if opts.batchSize <= 0 {
		opts.batchSize = 500
	}
...
func verify(ctx context.Context, source *sql.DB, target *sql.DB) error {
	for _, table := range roomTables {
		...
		if sourceCount != targetCount || sourceHash != targetHash {
			...
			return fmt.Errorf("room database verification failed")
		}
