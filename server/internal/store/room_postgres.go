...
func NewPostgresRoomStore(db *gorm.DB) *PostgresRoomStore {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.WithContext(ctx).Raw(
			...
			if isUniqueViolation(err) || errors.Is(err, gorm.ErrDuplicatedKey) {
				return roomapi.ErrRoomCodeExists
			}
...
func findActiveMembers(ctx context.Context, tx *gorm.DB, roomID string) ([]roomapi.Member, error) {
	...
	`
	rows, err := tx.WithContext(ctx).Raw(query, roomID).Rows()
	if err != nil {
...
func findActiveRoomByCode(ctx context.Context, tx *gorm.DB, roomCode string) (roomapi.Room, error) {
	const query = `
		SELECT id::text, room_code, host_user_id::text, media_episode_id::text, status
		FROM rooms
		WHERE room_code = ? AND status IN ('active', 'grace_period')
	`
	var room roomapi.Room
	if err := tx.WithContext(ctx).Raw(query, roomCode).Row().Scan(
		&room.ID,
		&room.RoomCode,
		&room.HostUserID,
		&room.MediaItemID,
...
func findActiveMember(ctx context.Context, tx *gorm.DB, roomID string, userID string) (roomapi.Member, error) {
	...
	`
	var member roomapi.Member
	if err := tx.WithContext(ctx).Raw(query, roomID, userID).Row().Scan(&member.UserID, &member.Role); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func reactivateRoomMember(ctx context.Context, tx *gorm.DB, roomID string, userID string) error {
	const updateExisting = `
		...
		WHERE id = (
			SELECT id
			FROM room_members
			WHERE room_id = ? AND user_id = ? AND is_active = false
	...
	if err := tx.WithContext(ctx).Raw(updateExisting, roomID, userID).Row().Scan(&restoredUserID); err != nil {
		if !errors.Is(err, sql.ErrNoRows) {
...
func insertRoomMember(ctx context.Context, tx *gorm.DB, roomID string, userID string, role string) error {
	const query = `
		INSERT INTO room_members (room_id, user_id, role, is_active)
		VALUES (?, ?, ?, true)
	`
	if err := tx.WithContext(ctx).Exec(query, roomID, userID, role).Error; err != nil {
		if isUniqueViolation(err) || errors.Is(err, gorm.ErrDuplicatedKey) {
			return nil
		}
...
func rowExists(ctx context.Context, tx *gorm.DB, query string, args ...any) (bool, error) {
	var marker int
	if err := tx.WithContext(ctx).Raw(query, args...).Row().Scan(&marker); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return false, nil
		}
		return false, err
...
func (s *PostgresRoomStore) MarkRoomGracePeriod(
	...
	const query = `
		...
		WHERE room_code = ? AND status IN ('active', 'grace_period')
	`
	if err := s.db.WithContext(ctx).Exec(query, lastEmptyAt, destroyAfter, roomCode).Error; err != nil {
...
func (s *PostgresRoomStore) MarkRoomActive(ctx context.Context, roomCode string) error {
	const query = `
		...
		WHERE room_code = ? AND status IN ('active', 'grace_period')
	`
	if err := s.db.WithContext(ctx).Exec(query, roomCode).Error; err != nil {
...
func (s *PostgresRoomStore) DestroyRoom(ctx context.Context, roomCode string) error {
	const query = `DELETE FROM rooms WHERE room_code = ?`
	if err := s.db.WithContext(ctx).Exec(query, roomCode).Error; err != nil {
...
func (s *PostgresRoomStore) MarkAllActiveRoomsGracePeriod(
	...
	const query = `
		...
		WHERE status = 'active'
	`
	result := s.db.WithContext(ctx).Exec(query, lastEmptyAt, destroyAfter)
	if result.Error != nil {
		return 0, fmt.Errorf("mark all active rooms grace_period: %w", result.Error)
	}
