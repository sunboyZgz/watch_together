...
Next phases keep the hybrid route: Phase 14 validates `room-authority-service` through `rpc-pilot`, and Phase 15+ can introduce command inbox or command ingress only if durable command audit becomes necessary.

## Phase 14 Room Authority RPC Pilot

Phase 13 adds the design boundary for `room-authority-service`; see [Room Authority Service Design](./room-authority-service-design.md). Phase 14 implements the internal authority RPC and `cmd/roomauthorityservice` for the `rpc-pilot` profile.
...
The target design keeps `roomserver` as the client edge while moving authority decisions toward `roomId` actors. Phase 14 adds `authority.proto`, `cmd/roomauthorityservice`, and `AUTHORITY_SERVICE_MODE=rpc` for `rpc-pilot`; app/prod defaults intentionally do not route authority traffic through the service.
