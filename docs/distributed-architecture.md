...
This route deliberately merges design and implementation work. New slices should continue to ship with the real Android-facing handler path unchanged, internal RPC/local adapters, compose wiring, owner-boundary updates, and a business smoke proving the user path. After Phase 23, the main remaining target-architecture gap is operational cutover and cleanup: production authority RPC canary, stronger migration/backup runbooks, and eventual shadow-table removal after confidence is high.
