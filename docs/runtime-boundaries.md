...
This document records the runtime ownership boundaries for the room system. Phase 4 adds a `distributed_authority` MVP for multi-instance room authority, Phase 5 adds fenced authority recovery from the Kafka canonical timeline, Phase 6 adds distributed seek rate limiting plus observability, Phase 7 adds service foundation boundaries with optional media/timeline RPC adapters and logical database ownership, Phase 8 closes the media table access boundary for room/progress while making the RPC pilot verifiable, Phase 9 adds an independent media database boundary through `MEDIA_DATABASE_URL`, Phase 10 adds an independent timeline database boundary through `TIMELINE_DATABASE_URL`, Phase 11 makes timeline RPC the default compose path while keeping timeline workers as separate timeline-owned processes, Phase 12 moves result timeline ownership into `cmd/timelineservice`, Phase 13 documents the future `room-authority-service` boundary, Phase 14 implements a non-default authority RPC pilot in `rpc-pilot`, Phase 15 hardens that pilot with readiness, metrics, identity, and failure-semantics checks, Phase 16 makes local compose `app` default to media, timeline, and authority RPC, Phase 17/18 adds `cmd/identityservice`, Phase 19 adds `cmd/roomservice` for room metadata and membership RPC, Phase 20 adds progress/home composition RPC plus `PROGRESS_DATABASE_URL`, Phase 21 adds `ROOM_DATABASE_URL` plus room lifecycle/recovery RPC, Phase 22 adds `IDENTITY_DATABASE_URL` plus home composition SQL read-model removal, Phase 23 adds the full-RPC multi-database smoke gate, Phase 25 promotes authority decisions into `authority.Engine`, Phase 26 adds `cmd/apigateway` as the public REST/BFF edge, Phase 27 removes the main-database shadow-table path, Phase 28 adds WebSocket drain plus Android automatic reconnect for rolling release semantics, Phase 29 verifies rolling release recovery in Compose with a smoke-only second `roomserver` plus rolling Nginx `/ws` upstream, Phase 29.5 moves the Compose Kafka baseline to `apache/kafka:3.7.0` before kind, and Phase 30 adds local `kind` + `Kustomize` orchestration with NodePort and two `roomserver` replicas for rolling restart verification. Local compose and production now use the same RPC-only service shape: `apigateway` owns public REST, `roomserver` owns WebSocket/session gateway behavior, and owner services own their databases.
...
| WebSocket connection objects | Local Go process memory | Must remain local. Rolling releases use graceful drain, client reconnect, and lazy bootstrap/recovery rather than transparent connection migration. |
...
| Internal RPC | ConnectRPC helper layer | Local compose app and production default identity, room, media, progress, home, timeline, and authority to RPC. Local adapters are compatibility paths only and still require owner databases when they touch durable state. |
...
| Database ownership | Owner PostgreSQL databases | Main database migrations no longer create owner tables. Identity, room, media, progress, and timeline tables live only in their owner databases. |
| Room metadata and membership | Room service + room PostgreSQL database | Durable business state selected by required `ROOM_DATABASE_URL`; lifecycle and recovery metadata go through `RoomInternalService` in compose RPC mode. |
| Media metadata | Media PostgreSQL database | Durable catalog state selected by required `MEDIA_DATABASE_URL`. |
| Timeline outbox | Timeline PostgreSQL database | Durable outbox state selected by required `TIMELINE_DATABASE_URL`; default compose access is through `cmd/timelineservice` and timeline-owned workers. |
...
Current note: local compose `app` and production compose run `cmd/apigateway`, `cmd/identityservice`, `cmd/roomservice`, `cmd/mediaservice`, `cmd/progressservice`, `cmd/homecompositionservice`, `cmd/timelineservice`, and `cmd/roomauthorityservice` by default. Phase 30 uses the same service set in `kind`, with `roomserver` scaled to two replicas behind a NodePort while `/ws` remains pinned to the `roomserver` Service. `apigateway` owns Android-facing REST/BFF routes and does not receive business database URLs. `roomserver` owns `/ws`, WebSocket connection tables, heartbeat, send queues, drain state, and session runtime mirrors. In authority RPC mode, `roomauthorityservice` owns the control decision engine and broadcasts accepted envelopes through NATS; WebSocket connections remain inside `roomserver`.
...
Timeline PostgreSQL `room_timeline_outbox` stores canonical timeline result events. It lives in the timeline database selected by `TIMELINE_DATABASE_URL`. `cmd/outboxworker` publishes it to Kafka, and `cmd/derivedworker` derives control-result and membership topics. Online WebSocket fan-out remains NATS + local connection tables.
...
Old authority results are fenced by epoch. Local apply, NATS control replies, and NATS broadcasts are accepted only when the observed Redis authority lease is still active for the expected instance and epoch. `WS_CONTROL_REQUEST_TIMEOUT_MS` bounds how long the session gateway waits for a forwarded authority decision; compose uses a cold-start-safe `10000` ms budget so first-control room/timeline recovery can complete before failing closed. A recovering room returns `room authority recovering` or a current snapshot; WebSocket connections are never moved between instances.

Phase 5 hardening requires accepted controls in `distributed_authority` to write `room_timeline_outbox` before Redis snapshot writes or WebSocket/NATS broadcast. If the outbox write fails, the accepted envelope is not broadcast and the client receives an error or current `room_state`. Rejected controls remain best-effort outbox writes. The timeline boundary is fail-closed when timeline RPC or `TIMELINE_DATABASE_URL` is unavailable.
...
`GET /healthz` remains lightweight liveness. `GET /readyz` reports JSON dependency readiness for PostgreSQL, Redis, NATS, Kafka, outbox, recovery, RPC dependencies, and `roomserver_draining`. During drain, `/readyz` returns `not_ready` while `/healthz` remains `ok`. `GET /metrics` exposes Prometheus metrics when enabled. `roomserver` exposes metrics on the main HTTP server; worker processes expose metrics only when `METRICS_ADDR` is set. The observability module only records and exposes runtime state; it does not participate in room authority, playback decisions, or recovery decisions.
...
`IDENTITY_SERVICE_MODE`, `ROOM_SERVICE_MODE`, `MEDIA_SERVICE_MODE`, `PROGRESS_SERVICE_MODE`, `HOME_SERVICE_MODE`, `TIMELINE_SERVICE_MODE`, and `AUTHORITY_SERVICE_MODE` accept `local` or `rpc`. `local` keeps in-process compatibility adapters, but durable adapters still use owner database URLs. `rpc` calls `cmd/identityservice`, `cmd/roomservice`, `cmd/mediaservice`, `cmd/progressservice`, `cmd/homecompositionservice`, `cmd/timelineservice`, or `cmd/roomauthorityservice` over ConnectRPC. `SERVER_EDGE_MODE` selects `combined`, `api_gateway`, or `session_gateway`; compose runs `apigateway` in `api_gateway` mode and `roomserver` in `session_gateway` mode. Phase 8 uses generated typed ConnectRPC contracts from `server/api/internal/v1` into `server/internal/rpcgen/v1`; Android HTTP/WebSocket payloads are unchanged. Phase 11 makes timeline RPC the default compose path: roomserver records and reads timeline data through `cmd/timelineservice`, while `cmd/outboxworker` and `cmd/derivedworker` remain separate timeline-owned workers. Local compose `app` and production compose now default to identity, room, media, progress, home, timeline, and authority RPC; roomserver `/readyz` actively probes `identity_rpc`, `room_rpc`, `media_rpc`, `progress_rpc`, `home_rpc`, `timeline_rpc`, and `authority_rpc`. Phase 27 makes this path executable from clean volumes with owner-only schema: migrations for main/identity/room/media/progress/timeline are applied before the app stack starts, and smoke assertions verify owner DB writes plus absent main-database owner tables. Phase 25 makes the production authority RPC path use `authority.Engine` rather than `transport.WebSocketHandler`. Phase 26 routes public REST through `apigateway` and `/ws` through `roomserver`; Phase 28 makes `roomserver` reject new WebSockets and close existing sessions with `1012 server draining` during shutdown, relying on Android reconnect plus lazy bootstrap/recovery. Phase 29 adds a `rolling-smoke` compose profile with `roomserver-rolling` and `nginx-rolling` so local verification can prove `/ws` failover between two session gateways while REST remains on `apigateway`. Phase 29.5 uses `apache/kafka:3.7.0` plus Apache Kafka env names in compose and checks the local image set before Phase 30. Phase 30 adds `kind`/`Kustomize` manifests with static Kubernetes Service DNS, NodePort `30080`, two `roomserver` replicas, readiness/liveness probes, migration Jobs, and a rolling restart smoke gate.
...
Phase 21 extends `RoomInternalService` to room lifecycle and recovery metadata. In compose RPC mode, `roomserver` backfills active rooms to grace period, writes grace-period/reactivation/destroy transitions, cleans expired rooms, and scans recoverable room codes through `cmd/roomservice`. `cmd/roomauthorityservice` also loads runtime bootstrap through room RPC. `PostgresRoomStore` no longer reads `users`; user validation and member profile enrichment use identity port/RPC.
...
Table owners and cross-context access rules are documented in [Database Ownership](./database-ownership.md) and enforced from `server/internal/store/db_ownership.yaml`. Phase 27 additionally requires main migrations to avoid owner tables.
...
`IDENTITY_DATABASE_URL` is required by `cmd/identityservice` and by any explicit local identity adapter that touches durable identity data. In `IDENTITY_SERVICE_MODE=rpc`, `roomserver` and `apigateway` call `cmd/identityservice` and do not connect to the identity database directly.

Identity schema migrations live in `server/identity_migrations`. The main database does not create `users`. `cmd/identitydbsync` is a legacy explicit import tool and requires `--source-database-url`.
...
`MEDIA_DATABASE_URL` is required by `cmd/mediaservice` and by any explicit local media adapter that touches durable media data. In `MEDIA_SERVICE_MODE=rpc`, `roomserver` and `apigateway` call `cmd/mediaservice` and do not connect to the media database directly.

Media schema migrations live in `server/media_migrations`. Main-database migrations live in `server/migrations` and do not create or reference media owner tables.
...
`cmd/mediadbsync` is a legacy explicit import tool and requires `--source-database-url`.
...
`TIMELINE_DATABASE_URL` is required by `cmd/timelineservice`, `cmd/outboxworker`, and any explicit local timeline adapter that touches durable timeline data. In `TIMELINE_SERVICE_MODE=rpc`, `roomserver` and `cmd/roomauthorityservice` call `cmd/timelineservice` and do not connect to the timeline database directly.

Timeline schema migrations live in `server/timeline_migrations`. Main-database migrations live in `server/migrations` and do not create `room_timeline_outbox`. Development-stage timeline databases start empty unless explicitly imported.

Timeline availability is fail-closed. If `TIMELINE_DATABASE_URL` or timeline RPC is unavailable, accepted distributed controls do not finalize accepted request idempotency and do not broadcast accepted WebSocket envelopes. Recovery also must not complete an authority epoch from an unavailable timeline recovery feed.
...
## Phase 21 Room Database Boundary

Phase 21 adds:

```text
ROOM_DATABASE_URL=
ROOM_POSTGRES_DB=anime_watch_room_dev
```

`ROOM_DATABASE_URL` is required by `cmd/roomservice` and by any explicit local room adapter that touches durable room data. In `ROOM_SERVICE_MODE=rpc`, `roomserver`, `apigateway`, and `cmd/roomauthorityservice` call `cmd/roomservice` and do not connect to the room database directly.

Room schema migrations live in `server/room_migrations`. The room database keeps `host_user_id` and `media_episode_id` as durable ids but does not define cross-database foreign keys to identity or media tables. `cmd/roomdbsync` is a legacy explicit import tool and requires `--source-database-url`.
...
For explicit compatibility testing, set `ROOMSERVER_TIMELINE_SERVICE_MODE=local` and provide `ROOMSERVER_TIMELINE_DATABASE_URL` in compose so only roomserver receives the direct timeline DB URL. This is still owner-DB-backed; it is not a main-database fallback.
...
## Phase 16/26 Local App Full RPC Path

Phase 14 adds the generated authority RPC and `cmd/roomauthorityservice` for the `rpc-pilot` profile. Phase 15 hardens that pilot for cutover readiness. Phase 16 promotes the same service into local compose `app`, and Phase 25 promotes it into production compose by default. Phase 17/18 adds `cmd/identityservice` to the same local app RPC baseline. Phase 26 adds `cmd/apigateway` as the public REST edge and makes roomserver a session gateway in compose.

- Keep `apigateway` as the public REST/BFF edge in compose.
- Keep `roomserver` as the WebSocket/session edge with local connection tables.
...
- Keep the roomserver local/NATS authority behavior as an explicit rollback path and parity baseline.
...
Compatibility mode in compose is explicit and still needs owner DB URLs:
...
- WebSocket graceful drain plus Android reconnect to any healthy `roomserver`.
...
- Transparent WebSocket/TCP connection migration between instances. The supported production behavior is drain plus reconnect.
