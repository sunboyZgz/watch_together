...
PostgreSQL is still the durable storage technology, but business data is split by owner database in the default path. Identity, room, media, progress, and timeline use `IDENTITY_DATABASE_URL`, `ROOM_DATABASE_URL`, `MEDIA_DATABASE_URL`, `PROGRESS_DATABASE_URL`, and `TIMELINE_DATABASE_URL`; those services do not fall back to the main `DATABASE_URL`. The main database no longer creates the owner tables. The in-process room manager owns live WebSocket connection objects. Redis stores runtime coordination state when configured: latest snapshots, authority leases, active-device leases, request idempotency, presence, and distributed seek rate limiting. Kafka stores the durable room timeline result log, and NATS Core handles realtime cross-instance fan-out and control forwarding.
...
The recommended local and production shape is full RPC: `apigateway` serves Android-facing REST, `roomserver` serves WebSocket/session gateway traffic, and business state is accessed through identity, room, media, progress, home, timeline, and authority RPC services. `distributed_authority` enables Redis authority leases, NATS forwarding, Kafka timeline logging, authority recovery, distributed seek rate limiting, and Prometheus/readiness observability. Phase 27 removes the main-database shadow table path so local debugging matches production service boundaries. See [Runtime Boundaries](./runtime-boundaries.md), [Distributed Architecture](./distributed-architecture.md), [Database Ownership](./database-ownership.md), and [Room Authority Service Design](./room-authority-service-design.md) for the current architecture and labeled future boundary.
