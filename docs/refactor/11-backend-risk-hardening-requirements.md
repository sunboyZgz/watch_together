...
- requestId 去重能处理重试重复。
- 服务端已经有按房间生效的 `seek` 最小间隔保护，默认 250ms。
- 超过频率的 seek 不推进权威 timeline，不广播控制事件，只向发起端返回最新 `room_state`。

目标行为：

- 客户端只在明确用户操作时发送控制事件。
- seek 建议在拖动结束后发送，而不是拖动中高频发送。
- 服务端第一版采用短窗口拒绝策略，避免把拖动中的高频 seek 放大成全房间广播。
- 所有控制事件统一写入权威 timeline 并递增 seq。

实现前需确认：

- Android 当前 seek 发送时机是否已经是拖动结束。
- Android 当前进度条 seek 发送时机已是拖动结束；服务端节流仍作为异常客户端和压测保护。
...
4. 控制事件频率压测和 seek 节流评估。已完成首轮：`seek` 默认 250ms/room 最小间隔，压测脚本支持 `CONTROL_PATTERN=seek`。
